...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/03/31 04:39:55 by jtaylor           #+#    #+#             */
...
const uint32_t	g_sha_224[] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
...
static int		sha224_prepross(char *init_mg, size_t len, t_ssl *t)
{
	int i;
	...
	ft_strcpy((char *)t->tmp_sha_ptr, init_mg);
...
static void		schedule_array224(t_ssl *t, int i)
{
	int j;
...
static void		sha224_algo(t_ssl *t, int j)
{
	t->tmp_sha1 = rigthrotat(t->e, 6) ^ rigthrotat(t->e, 11)
		^ rigthrotat(t->e, 25);
	t->tmp_sha2 = (t->e & t->f) ^ ((~t->e) & t->g);
	t->tmp_sha3 = t->h + t->tmp_sha1 + t->tmp_sha2 + g_sha_224[j] + t->tmp[j];
...
int				sha224(char *init_mg, size_t len, t_ssl *t)
{
	int i;
	int j;

	sha224_prepross(init_mg, len, t);
	i = 0;
	while (i < t->offset)
	{
		schedule_array224(t, i);
		j = -1;
		while (++j < 64)
			sha224_algo(t, j);
		t->h0 += t->a;
